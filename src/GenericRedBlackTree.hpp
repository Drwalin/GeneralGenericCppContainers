...
namespace Generic {
	...
	namespace Prototypes {
		...
		struct Node {
			...
			inline static KeyType Key(Tree* tree, void* node);
		};
	}
	...
	struct NodeImpl {
		...
		using KeyType = typename NodeAccessor::KeyType;
// 		using KeyTypeNoRef = typename std::remove_reference<typename NodeAccessor::KeyType>::type;
// 		using KeyTypeRef = KeyTypeNoRef&;
		...
		inline KeyType Key(Tree* tree);
		inline int Compare(Tree* tree, KeyType rightKey);
		...
		inline Node* FindGreaterEqual(Tree* tree, KeyType key);
		...
		inline uint64_t VerifyParenting(bool print, Tree* tree, uint64_t d=0, char C='R', void* parent_=NULL) {
			Node* parent = (Node*)parent_;
			if(print) {
				for(int i=0; i<d; ++i)
					printf("    ");
				printf(" %c %p  : parent (%p)\n", C, (void*)this, (void*)parent);
			}
...
// 		using KeyTypeNoRef = typename Node::KeyTypeNoRef;
// 		using KeyTypeRef = typename Node::KeyTypeRef;
		...
		template<typename T=Node>
		inline T* FindGreaterEqual(KeyType key) {
			if(Root())
				return (T*)(Root()->FindGreaterEqual(tree, key));
			return NULL;
		}
